...
std::string TPathTool::File_ExtractDir(const std::string &fileName, bool keepTrailSlash)
{
...
// -Returns folder path with trailing backslash, if requested
// -"keepTrailSlash" defaults to true
std::wstring TPathTool::File_ExtractDir(const std::wstring &fileName, bool keepTrailSlash)
{
...
bool TPathTool::Path_IsRelative(const std::string &path)
{
...
// -Static
// -Returns true if: no drive found, not a network path, and doesn't start with an environment var.
bool TPathTool::Path_IsRelative(const std::wstring &path)
{
...
std::string TPathTool::Path_Combine(const std::string& path1, const std::string& path2)
{
	...
	if (path1.length() == 0)
		...
		return path1;
	else if (((path1[path1.length() - 1] == '\\' || path1[path1.length() - 1] == '/') && (path2[0] != '\\' && path2[0] != '/')) ||
		((path1[path1.length() - 1] != '\\' && path1[path1.length() - 1] != '/') && (path2[0] == '\\' || path2[0] == '/')))
...
bool TPathTool::Path_ExpandEnvironmentVars(const std::string& path, std::string &dest)
{
	...
	if (nCharsWritten > buffSize)
	{
		...
		buff = new char[buffSize + sizeof('\0') + 1];
	...
	if (nCharsWritten == 0)
	{
		...
		std::wstring errStr = GetWindowsLastErrorCodeAsStringW(winLastErrTmp);
		ELog.fwprintf(ELogMsgLevel::LML_Light, L"%sError: ExpandEnvironmentStringsA() failed for: \"%s\" (0x%08lx): %s\n",
			codeSectionStr, Utf8ToUnicodeStr(path).c_str(), winLastErrTmp, errStr.c_str());
#endif
		return false;
	}
...
bool TPathTool::Path_ExpandEnvironmentVars(const std::wstring& path, std::wstring &dest)
{
	...
	if (nCharsWritten > buffSize)
	{
		...
		delete[] buff;
		buffSize = static_cast<size_t>(nCharsWritten) + 1;
		buff = new wchar_t[buffSize + sizeof(L'\0')];
